...
# Task1: Improve this script to also retrieve and install the files kept in the https://zonzorp.net/pics.tgz tarfile
...
# downlaod a zipfile of pictures to our Pictures directory - assumes you are online
# unpack the download zipfile if the download succeeded - assumes we have an unzip command on this system
# delete the local copy of the zipfile after a successful unpack of the zipfile
wget -q -o ~/public_html/pics/pics.zip https://zonzorp.net/pics.zip && unzip -d ~/public_html/pics -o -q ~/public_html/pics/pics.zip && rm ~/public_html/pics/pics.zip
...
Found $(find ~/public_html/pics -type f/wc -1) files in the public_html/pics directory.
